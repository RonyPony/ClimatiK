...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Login')), body: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
      ...
      ElevatedButton(onPressed: state.isLoading?null:() => ref.read(authControllerProvider.notifier).login(email.text, pass.text), child: const Text('Entrar')),
      TextButton(onPressed: ()=>context.push('/register'), child: const Text('Crear cuenta')),
      if (state.hasError) Text(state.error.toString())
    ]))),
    floatingActionButton: state.value != null ? FloatingActionButton(onPressed: ()=>ref.read(authControllerProvider.notifier).logout(), child: const Icon(Icons.logout)) : null,
    );
  }
}
